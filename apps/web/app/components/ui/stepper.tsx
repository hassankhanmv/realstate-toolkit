...
function useStep() {
  ...
  if (!context) throw new Error("useStep must be used within a <StepperItem />")
  return context
}
  ...
  ({ activeStep, onStepClick, className, children, ...props }, ref) => {
    return (
      <StepperContext.Provider value={{ activeStep, onStepClick }}>
        <div ref={ref} className={cn("relative flex items-center justify-between w-full px-6", className)} {...props}>
    ...
    return (
      <StepContext.Provider value={{ step, state }}>
        <div
          ...
          className={cn("flex flex-col items-center gap-2 bg-card px-3 cursor-pointer group", className)}
          {...props}
        >
          {children}
        </div>
    ...
    return (
      <div
        ...
        className={cn(
          ...
          {
            "border-accent bg-accent text-accent-foreground shadow-md": state === "active",
            "border-primary bg-primary text-primary-foreground": state === "completed",
            "border-destructive/60 bg-destructive/10 text-destructive": state === "error",
            "border-border bg-card text-muted-foreground group-hover:border-primary/50": state === "inactive",
          },
          className
        )}
        {...props}
      ...
      <span
        ...
        className={cn(
          "text-xs font-semibold uppercase tracking-wider transition-colors",
          {
            ...
            "text-muted-foreground group-hover:text-foreground/70": state === "inactive",
          },
          className
        )}
        {...props}
